...
def load_and_clean_data():
    ...
    try:
        ...
        print(f"Kesalahan saat memproses data: {e}")
        raise
...
def train_and_export_models(merged_data):
    ...
    def graduation_probability(row):
        if str(row['status_mahasiswa']).lower() == 'lulus':
            return 1.0
        elif str(row['status_mahasiswa']).lower() == 'aktif':
            return np.clip((row['ipk_mahasiswa'] + row['nilai_rata_rata']) / 8, 0, 1)
        else:
            return 0.0

    y_graduation = merged_data.apply(graduation_probability, axis=1)
